defmodule PcoApi.People.SchoolOptionTest do
  ...
  test ".self retrieves the details of a Workflow when passed a single record", %{bypass: bypass} do
    ...
    school_options = %PcoApi.Record{
      links: %{"self" => "https://api.planningcenteronline.com/people/v2/school_options/1"}
    }
  ...
  test ".promotes_to_school gets a SchoolOption record with a promotes_to_school link", %{
    bypass: bypass
  } do
    Bypass.expect(bypass, fn conn ->
