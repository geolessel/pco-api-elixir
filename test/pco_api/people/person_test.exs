defmodule PcoApi.People.PersonTest do
  ...
  test ".self retrieves the details of a Person when passed a single record", %{bypass: bypass} do
    ...
    me = %PcoApi.Record{
      links: %{"self" => "https://api.planningcenteronline.com/people/v2/people/1"}
    }
  ...
  test ".new with attributes builds a PcoApi.Record" do
    expected = %PcoApi.Record{
      attributes: %{"first_name" => "geo", "last_name" => "lessel"},
      type: "Person"
    }
  ...
  def new_person do
    %PcoApi.Record{attributes: %{"first_name" => "geo", "last_name" => "lessel"}, type: "Person"}
  end
end
